...
After dragging and dropping the collected data into BloodHound, you might encounter an "Incompatible Collector" error. This usually happens due to a version mismatch between the SharpHound collector on the target machine and BloodHound on your analysis machine.

Additionally, some folks report that newer BloodHound versions require running the .exe version of SharpHound instead of the .ps1 script. Using the PowerShell version may also trigger the "Incompatible Collector" error.
